import { LitElement, html, css, nothing } from 'lit';

/**
 * MD3 Bottom Sheet component.
 *
 * Variants:
 * - `standard`: Non-modal, coexists with page content. Stays in document flow.
 * - `modal`: Overlays with scrim. Fixed positioning, blocks interaction with content behind.
 *
 * Supports drag-to-dismiss on the drag handle.
 */
export class MdBottomSheet extends LitElement {
  static override properties = {
    open: { type: Boolean, reflect: true },
    variant: { type: String, reflect: true },
    showDragHandle: { type: Boolean, attribute: 'show-drag-handle' },
  };

  declare open: boolean;
  declare variant: 'standard' | 'modal';
  declare showDragHandle: boolean;

  private _startY = 0;
  private _currentY = 0;
  private _dragging = false;

  constructor() {
    super();
    this.open = false;
    this.variant = 'modal';
    this.showDragHandle = true;
  }

  static override styles = css`
    :host {
      display: contents;
    }

    /* Modal overlay */
    .scrim {
      position: fixed;
      inset: 0;
      z-index: 999;
      background: var(--md-sys-color-scrim, #000);
      opacity: 0;
      visibility: hidden;
      transition: opacity 300ms cubic-bezier(0.2, 0, 0, 1),
                  visibility 0s 300ms;
      pointer-events: none;
    }

    :host([variant='modal'][open]) .scrim {
      opacity: 0.32;
      visibility: visible;
      pointer-events: auto;
      transition: opacity 300ms cubic-bezier(0.2, 0, 0, 1);
    }

    /* Sheet container */
    .sheet {
      background: var(--md-sys-color-surface-container-low, #f7f2fa);
      border-radius: 28px 28px 0 0;
      box-shadow: var(--md-sys-elevation-level1,
        0px 1px 3px 1px rgba(0,0,0,0.15),
        0px 1px 2px 0px rgba(0,0,0,0.3));
      display: flex;
      flex-direction: column;
      max-height: 70vh;
      overflow: hidden;
      transform: translateY(100%);
      transition: transform 300ms cubic-bezier(0.2, 0, 0, 1);
      will-change: transform;
    }

    :host([variant='modal']) .sheet {
      position: fixed;
      bottom: 0;
      left: 0;
      right: 0;
      z-index: 1000;
    }

    :host([variant='standard']) .sheet {
      position: relative;
    }

    :host([open]) .sheet {
      transform: translateY(0);
    }

    .sheet.dragging {
      transition: none;
    }

    /* Drag handle */
    .drag-handle-area {
      display: flex;
      justify-content: center;
      padding: 16px 0 8px;
      cursor: grab;
      touch-action: none;
    }

    .drag-handle-area:active {
      cursor: grabbing;
    }

    .drag-handle {
      width: 32px;
      height: 4px;
      border-radius: 2px;
      background: var(--md-sys-color-on-surface-variant, #49454f);
      opacity: 0.4;
    }

    /* Content */
    .content {
      flex: 1;
      overflow-y: auto;
      padding: 0 24px 24px;
    }

    /* Reset slotted list background to match sheet surface */
    ::slotted(md-list) {
      background: transparent;
      --md-list-container-color: transparent;
    }
  `;

  override render() {
    return html`
      ${this.variant === 'modal'
        ? html`<div class="scrim" @click=${this._handleScrimClick}></div>`
        : nothing}
      <div class="sheet ${this._dragging ? 'dragging' : ''}">
        ${this.showDragHandle
          ? html`
            <div class="drag-handle-area"
              @pointerdown=${this._handleDragStart}
              @pointermove=${this._handleDragMove}
              @pointerup=${this._handleDragEnd}
              @pointercancel=${this._handleDragEnd}
            >
              <div class="drag-handle"></div>
            </div>`
          : nothing}
        <div class="content">
          <slot></slot>
        </div>
      </div>
    `;
  }

  override updated(changed: Map<string, unknown>) {
    if (changed.has('open')) {
      this.dispatchEvent(new CustomEvent('bottom-sheet-changed', {
        detail: { open: this.open },
        bubbles: true,
        composed: true,
      }));
    }
  }

  override connectedCallback() {
    super.connectedCallback();
    this._onKeyDown = this._onKeyDown.bind(this);
    document.addEventListener('keydown', this._onKeyDown);
  }

  override disconnectedCallback() {
    super.disconnectedCallback();
    document.removeEventListener('keydown', this._onKeyDown);
  }

  private _onKeyDown(e: KeyboardEvent) {
    if (e.key === 'Escape' && this.open && this.variant === 'modal') {
      this.open = false;
    }
  }

  private _handleScrimClick() {
    this.open = false;
  }

  private _handleDragStart(e: PointerEvent) {
    this._dragging = true;
    this._startY = e.clientY;
    this._currentY = 0;
    (e.target as HTMLElement).setPointerCapture(e.pointerId);
    this.requestUpdate();
  }

  private _handleDragMove(e: PointerEvent) {
    if (!this._dragging) return;
    this._currentY = Math.max(0, e.clientY - this._startY);
    const sheet = this.shadowRoot?.querySelector('.sheet') as HTMLElement;
    if (sheet) {
      sheet.style.transform = `translateY(${this._currentY}px)`;
    }
  }

  private _handleDragEnd() {
    if (!this._dragging) return;
    this._dragging = false;
    const sheet = this.shadowRoot?.querySelector('.sheet') as HTMLElement;

    // If dragged more than 100px down, dismiss
    if (this._currentY > 100) {
      this.open = false;
    }

    if (sheet) {
      sheet.style.transform = '';
    }
    this._currentY = 0;
    this.requestUpdate();
  }
}

customElements.define('md-bottom-sheet', MdBottomSheet);

declare global {
  interface HTMLElementTagNameMap {
    'md-bottom-sheet': MdBottomSheet;
  }
}
